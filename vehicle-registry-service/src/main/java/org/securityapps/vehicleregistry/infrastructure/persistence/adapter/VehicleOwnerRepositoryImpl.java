...
import org.securityapps.vehicleregistry.infrastructure.persistence.mapper.VehicleOwnerMapper;
import org.securityapps.vehicleregistry.infrastructure.persistence.repository.JpaVehicleOwnerRepository;
...
public class VehicleOwnerRepositoryImpl implements VehicleOwnerRepository {
    ...
    @Override
    public Optional<VehicleOwner> findVehicleOwnerByPhoneNumber(String phoneNumber){
        VehicleOwnerEntity entity=jpaVehicleOwnerRepository.findVehicleOwnerByAddressPhoneNumber(phoneNumber).orElseThrow(()->new ResourceNotFoundException("VehicleOwner not found"));
        return Optional.ofNullable(vehicleOwnerMapper.toDomain(entity));
    }
}
